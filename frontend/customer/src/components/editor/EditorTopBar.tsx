...
export function EditorTopBar({
  ...
  return (
    <header className="sticky top-0 z-30 w-full bg-surface/90 backdrop-blur-md border-b border-outline-variant/30">
      <div className="max-w-7xl w-full mx-auto flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-2.5 sm:py-3">
        {/* Back Button */}
        <button
        ...
        {autoSaving && (
          <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-surface-container text-on-surface-variant text-xs">
            <span className="w-2 h-2 rounded-full bg-primary animate-pulse" />
            <span>Menyimpan...</span>
          </div>
        )}

        {/* Action buttons: Status & Manual Save */}
        <button
          onClick={onChangeStatus}
          className={`hidden md:flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border transition-colors ${statusMeta.badge}`}
          title="Klik untuk mengubah status undangan"
        >
